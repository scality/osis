...
* If the entire Redis Sentinel cluster fails or crashes, the vCloud Director's OSE service must be restarted with the following command:
  ...
  * Once OSE service restart process is finished,
     * OSE will invoke `listS3Credentials` API to identify the keys for object storage operations. OSIS will return only an empty list for `listS3Credentials` API (as no key  is available for the user on Redis).
     * If no keys are present for a user, OSE will invoke `createS3Credentials` API to create a new key for object storage operations.
  * Access keys created by OSIS before OSE restart time (`ose-restart-time`) will not be listed by OSIS. vCloud Director Tenant Users are responsible to clean up unused access keys using S3 console or IAM API because they are the only one who knows if the access keys are in used or not.
  * Access key IDs generated by OSIS will be logged
...
4. Exclude the keys in the response that are not present in the Redis Sentinel.
